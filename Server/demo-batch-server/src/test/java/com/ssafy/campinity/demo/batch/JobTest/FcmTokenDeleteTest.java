...
public class FcmTokenDeleteTest {
    ...
    public void FcmTokenDeleteJobTest () throws Exception {
        ...
        FcmToken fcmToken1 = FcmToken.builder().token("testToken1").member(savedMember).expiredDate(LocalDate.now().plusMonths(1)).build();
        FcmToken ft1 = fcmTokenRepository.save(fcmToken1);

        FcmToken fcmToken2 = FcmToken.builder().token("testToken2").member(savedMember).expiredDate(date).build();
        FcmToken ft2 = fcmTokenRepository.save(fcmToken2);
        ...
        assertEquals(BatchStatus.COMPLETED, jobExecution.getStatus());  // job 실행 결과가 성공인지
