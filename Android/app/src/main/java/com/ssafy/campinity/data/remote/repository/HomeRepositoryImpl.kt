...
class HomeRepositoryImpl @Inject constructor(
    ...
    override suspend fun getHighestCampsite(): Resource<List<RankingCampsiteItem>> =
        wrapToResource(Dispatchers.IO) {
            homeRemoteDataSource.getHighestCampsite().map { it.toDomainModel() }
        }

    override suspend fun getHottestCampsite(): Resource<List<RankingCampsiteItem>> =
        wrapToResource(Dispatchers.IO) {
            homeRemoteDataSource.getHottestCampsite().map { it.toDomainModel() }
        }
}